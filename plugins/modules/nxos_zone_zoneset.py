...
def main():
    ...
    for eachZoneZonesetDetail in listOfZoneDetails:
        ...
        if shZoneStatusObj.isVsanAbsent():
            module.fail_json(msg='Vsan ' + str(vsan) + ' is not present in the switch. Hence cannot proceed.')

        if shZoneStatusObj.isLocked():
            module.fail_json(msg='zone has acquired lock on the switch for vsan ' + str(vsan) + '. Hence cannot proceed.')
